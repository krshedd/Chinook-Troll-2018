...
Make a skinny version of sport attributes. Need post-QA version of `KSPORT18.gcl` that has ASL joined.
```{r ONCOR_sport_attributes}
(sport_ASL <- read_csv(file = "../ASL Data/_2018_SEAK_SF_Whatman_AWL_10OCT18.csv") %>% 
   select(SITE, DATE, LENGTH, Whatman_Card, SAMPLE_NO, ScaleCardNumber, ScaleCardRow, Adipose_Clipped, TAGNUM, OTOLITH) %>% 
   mutate(WGC = str_pad(Whatman_Card, 10, "left", "0")) %>% 
   unite(fish_id, c("WGC", "SAMPLE_NO"), sep = "_", remove = FALSE))
