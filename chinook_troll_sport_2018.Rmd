...
Generate All Quad estimates for Early and Late Winter.
```{r summer_stratified_33RG}
SummerRet1_2018_33RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = 1:33, groupnames = GroupNames33, maindir = "../BAYES/Output/", mixvec = summer_2018_mixnames[c(2,4,1,3)], catchvec = c(38087, 14219, 2797, 3993), newname = "SummerRet1AllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

SummerRet2_2018_33RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = 1:33, groupnames = GroupNames33, maindir = "../BAYES/Output/", mixvec = summer_2018_mixnames[c(6,8,5,7)], catchvec = c(16907, 8649, 454, 1736), newname = "SummerRet2AllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

save_objects(c("SummerRet1_2018_33RG_StratifiedEstimatesStats", "SummerRet2_2018_33RG_StratifiedEstimatesStats"), "../Estimates objects")
```

Make in to tall tibble.
```{r summer_stratified_33RG_tibble}
# make in to a tidy tibble (tall)
summer_2018_33RG_stratified_estimates.tdy <- 
  bind_rows(
    lapply(c("SummerRet1AllQuad", "SummerRet2AllQuad"), function(mix) {  # loop over mixture names
      list("SummerRet1AllQuad" = SummerRet1_2018_33RG_StratifiedEstimatesStats, 
           "SummerRet2AllQuad" = SummerRet2_2018_33RG_StratifiedEstimatesStats)[[mix]] %>%  # for each matrix
        as_tibble(rownames = "group") %>%  # make tibble with rownames as group
        mutate(mixname = mix) %>%  # make column for mixname
        mutate(n_group = n_distinct(group))  # make column for number of groups
    } )
  ...
  gather(estimator, value, -mixname, -group, - n_group) %>%  # gather all estimators
  separate(mixname, c("fishery", "quadrant"), sep = 10) %>%  # extract district and gear
  mutate(estimator = factor(estimator, c("mean", "sd", "5%", "95%", "median", "P=0", "GR"))) %>%  # factor for ordering
  mutate(group = factor(group, GroupNames33))  # factor for ordering

save_objects(c("summer_2018_33RG_stratified_estimates.tdy"), "../Estimates objects")
```
...
# full 33 reporting groups
Sport_2018_33RG_EstimatesStats <- 
  CustomCombineBAYESOutput.GCL(groupvec = 1:33, groupnames = GroupNames33, maindir = "../BAYES/Output", mixvec = sport_2018_mixnames,
                               prior = "", ext = "RGN", nchains = 5, burn = 0.5, alpha = 0.1, PosteriorOutput = FALSE)

# dir.create("../Estimates objects")
save_objects(c("Sport_2018_33RG_EstimatesStats"), "../Estimates objects")
```

Check Gelman-Rubin
```{r sport_GR_33RG}
any(sapply(Sport_2018_33RG_EstimatesStats, function(mix) {mix[, "GR"]} ) > 1.2)
sum(sapply(Sport_2018_33RG_EstimatesStats, function(mix) {mix[, "GR"]} ) > 1.2)
sapply(Sport_2018_33RG_EstimatesStats, function(mix) {mix[, "GR"]} )
```
...
sport_2018_33RG_estimates.tdy <- 
  bind_rows(
    lapply(sport_2018_mixnames, function(mix) {  # loop over mixture names
      Sport_2018_33RG_EstimatesStats[[mix]] %>%  # for each matrix
        as_tibble(rownames = "group") %>%  # make tibble with rownames as group
        mutate(mixname = mix) %>%  # make column for mixname
        mutate(n_group = n_distinct(group))  # make column for number of groups
    } )
...
save_objects(c("sport_2018_33RG_estimates.tdy"), "../Estimates objects")
```

Just Alsek in NO quadrant, estimate is super low, so no big deal.
```{r sport_GR_33RG_tibble}
sport_2018_33RG_estimates.tdy %>% 
  spread(estimator, value) %>% 
  filter(GR > 1.2)
```
...
sport_2018_33RG_estimates.tdy %>% 
  ...
  arrange(mix, desc(mean)) %>% 
  select(mix, group, mean, sd, `5%`, `95%`)
```

# *ONCOR* Individual Assignment

Export genotypes to mixture files for *ONCOR*. This is for "Origins" in order to send the correct scale cards to respective agencies for aging. Using baseline and reporting group files from the "SEAK17" directory.
...
I will need the following files for input into *ONCOR*:
  ...
  * reporting group file: column 1 is pops, column 2 is reporting group
    1) "CTCgroups_21ProposalRG"
    2) "CTCgroups_30RG"
    3) "GAPSgroups_26RG")
...
Thu Jan 25 11:28:06 2018; spoke with Randy and he confirmed that we want ALL samples that have been genotyped (because they've already been sub-sampled with respect to harvest).

## Get pre-QA genotypes

Since we are going to run an all Sport and all Troll *ONCOR*, we need to grab the pre-QA genotypes (because we split in to smaller strata prior to QA).
```{r ONCOR_genotypes}
ONCOR_Mixtures <- c("KSPORT18", "KTROL17EW", "KTROL18LW", "KTROL18SP", "KTROL18SU")

save_objects("ONCOR_Mixtures", "../Objects")
load_sillys(path = "../Genotypes/original/", sillyvec = ONCOR_Mixtures)
```
...
save_objects(c("ONCOR_Mixtures_Miss", "ONCOR_Mixtures_Dups"), "../Objects")
```

## Create *ONCOR* input files

```{r ONCOR_input}
gcl2Genepop.GCL(sillyvec = ONCOR_Mixtures[1], path = "../ONCOR/Mixture/Sport2018.gen", loci = GAPSLoci, VialNums = TRUE, usat = TRUE)
gcl2Genepop.GCL(sillyvec = ONCOR_Mixtures[-1], path = "../ONCOR/Mixture/Troll2018.gen", loci = GAPSLoci, VialNums = TRUE, usat = TRUE)

# Remove "Pop" designations
rawdat <- scan(file = "../ONCOR/Mixture/Troll2018.gen", what = '', sep = '\n')
moddat <- rawdat[-grep(pattern = "Pop", x = rawdat)[-1]]
write.table(x = moddat, file = "../ONCOR/Mixture/Troll2018.gen", quote = FALSE, row.names = FALSE, col.names = FALSE)
```
...
rawsport <- scan(file = "../ONCOR/Output/sport2018_21RG_out.txt", what = '', sep = '\n', blank.lines.skip = FALSE)
skip <- grep(pattern = "PROBABILITY OF EACH INDIVIDUAL IN THE MIXTURE BELONGING TO EACH REPORTING GROUP", x = rawsport) + 1
(sport_dat <- read_delim(file = "../ONCOR/Output/sport2018_21RG_out.txt", delim = "\t", skip = skip, trim_ws = TRUE) %>% 
    dplyr::rename(FishID = X1))
```

Get 1st and 2nd most likely probabilities for each FishID
```{r sport_1st_2nd_probabilities}
# 1st probability RG
sport_dat_1 <- sport_dat %>% 
...
```{r sport_ONCOR_join_1st_2nd}
sport_dat_tdy <- left_join(x = sport_dat_1, y = sport_dat_2, by = "FishID") %>% 
  dplyr::rename(RG.1 = RG.x, prob.1 = prob.x, RG.2 = RG.y, prob.2 = prob.y) %>% 
  filter(prob.1 >= 0.8)
```

Get sport WGC, see if duplicates.
```{r sport_spring_summer_attributes, warning=FALSE}
sport_dna_key <- KSPORT18.gcl$attributes %>% 
  as_tibble() %>% 
  unite(fish_id, c("DNA_TRAY_CODE", "DNA_TRAY_WELL_CODE"), sep = "_", remove = FALSE) %>% 
  select(SillySource, fish_id)

table(table(sport_dna_key$fish_id, useNA = "always"), useNA = "always")
```

Get sport ASL and join with `sport_dna_key`.
```{r sport_ASL_ONCOR, message=FALSE, warning=FALSE}
sport_ASL <- read_csv("../ASL Data/_2018_SEAK_SF_Whatman_AWL_06MAY19.csv") %>% 
  select(SITE, DATE, LENGTH, Whatman_Card, SAMPLE_NO, ScaleCardNumber, ScaleCardRow, Adipose_Clipped, TAGNUM, otolithnumber) %>% 
  mutate(WGC = str_pad(Whatman_Card, 10, "left", "0")) %>% 
  unite(fish_id, c("WGC", "SAMPLE_NO"), sep = "_", remove = FALSE) %>% 
  right_join(sport_dna_key)
```

Join *ONCOR* with ASL and verify all fish have ASL data.
```{r sport_ONCOR_join_ALS}
(sport_ONCOR_dat <- sport_dat_tdy %>% 
   left_join(sport_ASL, by = c("FishID" = "SillySource")) %>% 
   mutate(Date = dmy(DATE)) %>% 
   dplyr::rename(IND = FishID, "Best Estimate" = RG.1, "Probability 1" = prob.1, "2nd Best Estimate" = RG.2, "Probability 2" = prob.2, "Port" = SITE, CardNo = ScaleCardNumber, IndNo = ScaleCardRow, Length = LENGTH, CWT = TAGNUM, AdClip = "Adipose_Clipped", Otolith = otolithnumber) %>% 
   select(IND, `Best Estimate`, `Probability 1`, `2nd Best Estimate`, `Probability 2`, Date, Port, CardNo, IndNo, Length, CWT, AdClip, Otolith))

sport_ONCOR_dat %>% filter(is.na(Port))
```

Save sport *ONCOR* results
```{r sport_ONCOR_save}
save_objects(objects = "sport_ONCOR_dat", path = "../Objects/")

# Canada
sport_ONCOR_canada <- sport_ONCOR_dat %>% 
  filter(`Best Estimate` %in% c("FraserEarly", "NorCentBC", "WCVI")) %>% 
  arrange(`Best Estimate`, desc(`Probability 1`))

# Washington
sport_ONCOR_washington <- sport_ONCOR_dat %>% 
  filter(`Best Estimate` %in% c("WACoast")) %>% 
  arrange(`Best Estimate`, desc(`Probability 1`))

# Oregon
sport_ONCOR_oregon <- sport_ONCOR_dat %>% 
  filter(`Best Estimate` %in% c("NORCoast")) %>% 
  arrange(`Best Estimate`, desc(`Probability 1`))

# Columbia
sport_ONCOR_columbia <- sport_ONCOR_dat %>% 
  filter(`Best Estimate` %in% c("IntColSuFa")) %>% 
  arrange(`Best Estimate`, desc(`Probability 1`))

# Alaska
sport_ONCOR_alaska <- sport_ONCOR_dat %>% 
  filter(`Best Estimate` %in% c("SSEAK")) %>% 
  arrange(`Best Estimate`, desc(`Probability 1`))


## Write out data for Anne
if(!dir.exists("../Origins")) {dir.create("../Origins")}

require(writexl)
sport_summary_list <- list("Canada" = sport_ONCOR_canada,
                           "Washington" = sport_ONCOR_washington,
                           "Oregon" = sport_ONCOR_oregon,
                           "Columbia" = sport_ONCOR_columbia,
                           "Alaska" = sport_ONCOR_alaska)

writexl::write_xlsx(x = sport_summary_list, path = "../Origins/Sport_2018_Scales_toread.xlsx", col_names = TRUE)
```

### Sport otoliths

Need a separate sheet with just WCVI fish to send to MTA Lab for otolith reading.
```{r sport_origins_WCVI_otoliths}
sport_ONCOR_canada %>% 
  filter(`Best Estimate` == "WCVI" & Port %in% c("SITKA", "CRAIG_KLAWOCK") & Otolith != ".") %>% 
  write_csv(path = "../Origins/Sport_2018_Otoliths_WCVI_toread.csv")
```

### Troll scales

Need to match up ASL data with *ONCOR* output. This is a bit of a pain in the ass, because Fish_ID = Dna.Specimen.No for Early and Late Winter, but not for Spring and Summer. So I need to re-join ASL to Spring and Summer Fish_ID.
```{r ONCOR_troll_raw_output}
rawtroll <- scan(file = "../ONCOR/Output/Troll2018_21RG_out.txt", what = '', sep = '\n', blank.lines.skip = FALSE)
skip <- grep(pattern = "PROBABILITY OF EACH INDIVIDUAL IN THE MIXTURE BELONGING TO EACH REPORTING GROUP", x = rawtroll) + 1
(troll_dat <- read_delim(file = "../ONCOR/Output/Troll2018_21RG_out.txt", delim = "\t", skip = skip, trim_ws = TRUE) %>% 
    dplyr::rename(FishID = X1))
```

Get 1st and 2nd most likely probabilities for each FishID
```{r troll_1st_2nd_probabilities}
# 1st probability RG
troll_dat_1 <- troll_dat %>% 
  gather(RG, prob, -FishID) %>% 
  group_by(FishID) %>% 
  slice(which.max(prob)) %>% 
  arrange(FishID)
...
```{r troll_ONCOR_join_1st_2nd}
troll_dat_tdy <- left_join(x = troll_dat_1, y = troll_dat_2, by = "FishID") %>% 
  dplyr::rename(RG.1 = RG.x, prob.1 = prob.x, RG.2 = RG.y, prob.2 = prob.y) %>% 
  filter(prob.1 >= 0.8)
```

Get troll Fish_IDs for Spring and Summer, see if duplicates.
```{r troll_spring_summer_attributes, warning=FALSE}
troll_dna_key <- bind_rows(KTROL17EW.gcl$attributes,
          KTROL18LW.gcl$attributes,
          KTROL18SP.gcl$attributes,
          KTROL18SU.gcl$attributes) %>% 
  mutate(`Dna Specimen No` = as.integer(paste0(str_sub(DNA_TRAY_CODE, 7, 10), str_pad(DNA_TRAY_WELL_CODE, 2, "left", "0")))) %>% 
...
table(table(troll_dna_key$`Dna Specimen No`, useNA = "always"), useNA = "always")
```

Get troll ASL and join with `troll_dna_key`.
```{r troll_ASL_ONCOR, message=FALSE, warning=FALSE}
troll_ASL <- read_csv("../ASL Data/20190204_troll_2017_2018_Detailed ASL Samples.csv") %>% 
  select(`Dna Specimen No`, `Sample Date`, `Port Code`, District, `Scale Card No`, `Specimen Number`, `Length mm`, `Cwt Strap Tag No`, `Adipose Clipped No Signal`) %>% 
  right_join(troll_dna_key)
```

Join *ONCOR* with ASL and verify all fish have ASL data.
```{r troll_ONCOR_join_ALS}
(troll_ONCOR_dat <- troll_dat_tdy %>% 
   left_join(troll_ASL, by = c("FishID" = "SillySource")) %>% 
   separate(col = `Sample Date`, into = c("Date", "trash"), sep = " ") %>% 
   mutate(Date = mdy(Date)) %>% 
   select(-trash) %>% 
   dplyr::rename(IND = FishID, "Best Estimate" = RG.1, "Probability 1" = prob.1, "2nd Best Estimate" = RG.2, "Probability 2" = prob.2, "Quad/Dist" = District, CWT = `Cwt Strap Tag No`, AdClipNoSignal = "Adipose Clipped No Signal") %>% 
   select(IND, `Dna Specimen No`, `Best Estimate`, `Probability 1`, `2nd Best Estimate`, `Probability 2`, Date, `Port Code`, `Quad/Dist`, `Scale Card No`, `Specimen Number`, `Length mm`, CWT, `AdClipNoSignal`))

troll_ONCOR_dat %>% filter(is.na(`Quad/Dist`))
```

Save troll *ONCOR* results
```{r troll_ONCOR_save}
save_objects(objects = "troll_ONCOR_dat", path = "../Objects/")

# Canada
troll_ONCOR_canada <- troll_ONCOR_dat %>% 
  filter(`Best Estimate` %in% c("FraserEarly", "NorCentBC", "WCVI")) %>% 
  arrange(`Best Estimate`, desc(`Probability 1`))

# Washington
troll_ONCOR_washington <- troll_ONCOR_dat %>% 
  filter(`Best Estimate` %in% c("WACoast")) %>% 
  arrange(`Best Estimate`, desc(`Probability 1`))

# Oregon
troll_ONCOR_oregon <- troll_ONCOR_dat %>% 
  filter(`Best Estimate` %in% c("NORCoast")) %>% 
  arrange(`Best Estimate`, desc(`Probability 1`))

# Columbia
troll_ONCOR_columbia <- troll_ONCOR_dat %>% 
  filter(`Best Estimate` %in% c("IntColSuFa")) %>% 
  arrange(`Best Estimate`, desc(`Probability 1`))

# Alaska
troll_ONCOR_alaska <- troll_ONCOR_dat %>% 
  filter(`Best Estimate` %in% c("SSEAK")) %>% 
  arrange(`Best Estimate`, desc(`Probability 1`))
...
require(writexl)
troll_summary_list <- list("Canada" = troll_ONCOR_canada,
                           "Washington" = troll_ONCOR_washington,
                           "Oregon" = troll_ONCOR_oregon,
                           "Columbia" = troll_ONCOR_columbia,
                           "Alaska" = troll_ONCOR_alaska)

writexl::write_xlsx(x = troll_summary_list, path = "../Origins/Troll_2018_Scales_toread.xlsx", col_names = TRUE)
```
...
```{r troll_mixtures}
troll_2018_mixnames <- c(winter_2018_mixnames, spring_2018_mixnames, summer_2018_mixnames)

Troll_2018_Mixtures_33RG_EstimatesStats <- 
  CustomCombineBAYESOutput.GCL(groupvec = 1:33, groupnames = GroupNames33, maindir = "../BAYES/Output", mixvec = troll_2018_mixnames,
                               prior = "", ext = "RGN", nchains = 5, burn = 0.5, alpha = 0.1, PosteriorOutput = FALSE)

Troll_2018_Mixtures_18RG_EstimatesStats <- 
  CustomCombineBAYESOutput.GCL(groupvec = GroupVec33RG_to18RG, groupnames = GroupNames18, maindir = "../BAYES/Output", mixvec = troll_2018_mixnames,
                               prior = "", ext = "RGN", nchains = 5, burn = 0.5, alpha = 0.1, PosteriorOutput = FALSE)

Troll_2018_Mixtures_8RG_EstimatesStats <- 
  CustomCombineBAYESOutput.GCL(groupvec = GroupVec33RG_to8RG, groupnames = GroupNames8, maindir = "../BAYES/Output", mixvec = troll_2018_mixnames,
                               prior = "", ext = "RGN", nchains = 5, burn = 0.5, alpha = 0.1, PosteriorOutput = FALSE)

Troll_2018_Mixtures_4RG_EstimatesStats <- 
  CustomCombineBAYESOutput.GCL(groupvec = GroupVec33RG_to4RG, groupnames = GroupNames4, maindir = "../BAYES/Output", mixvec = troll_2018_mixnames,
                               prior = "", ext = "RGN", nchains = 5, burn = 0.5, alpha = 0.1, PosteriorOutput = FALSE)

save_objects(objects = "troll_2018_mixnames", path = "../Objects/")
save_objects(objects = objects(pattern = "Troll_2018_Mixtures"), path = "../Estimates objects/")
```
...
troll_harvest <- bind_rows(
  read_csv("../Harvest Data/ft - Detailed Fish Tickets_traditional.csv"),
  read_csv("../Harvest Data/ft - Detailed Fish Tickets_spring.csv")) %>% 
  mutate(Month = month(`Date Fishing Ended`, label = TRUE, abbr = FALSE)) %>% 
  ...
  mutate(AY = case_when(Fishery == "Early Winter" & Year == 2017 ~ 2018,
                        Fishery == "Early Winter" & Year == 2018 ~ 2019,
                        TRUE ~ Year)) %>% 
  filter(AY == 2018)
...
EWint_2018_33RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = 1:33, groupnames = GroupNames33, maindir = "../BAYES/Output/", mixvec = winter_2018_mixnames[1:4], 
                          catchvec = c(4501, 86, 1262, 1549), newname = "EWintAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

EWint_2018_18RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to18RG, groupnames = GroupNames18, maindir = "../BAYES/Output/", mixvec = winter_2018_mixnames[1:4], 
                          catchvec = c(4501, 86, 1262, 1549), newname = "EWintAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

EWint_2018_8RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to8RG, groupnames = GroupNames8, maindir = "../BAYES/Output/", mixvec = winter_2018_mixnames[1:4], 
                          catchvec = c(4501, 86, 1262, 1549), newname = "EWintAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

EWint_2018_4RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to4RG, groupnames = GroupNames4, maindir = "../BAYES/Output/", mixvec = winter_2018_mixnames[1:4], 
                          catchvec = c(4501, 86, 1262, 1549), newname = "EWintAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

save_objects(objects = objects(pattern = "EWint_2018_"), path = "../Estimates objects/")
```
...
LWint_2018_33RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = 1:33, groupnames = GroupNames33, maindir = "../BAYES/Output/", mixvec = winter_2018_mixnames[5:8], 
                          catchvec = c(1664, 362, 903, 1640), newname = "LWintAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

LWint_2018_18RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to18RG, groupnames = GroupNames18, maindir = "../BAYES/Output/", mixvec = winter_2018_mixnames[5:8], 
                          catchvec = c(1664, 362, 903, 1640), newname = "LWintAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

LWint_2018_8RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to8RG, groupnames = GroupNames8, maindir = "../BAYES/Output/", mixvec = winter_2018_mixnames[5:8], 
                          catchvec = c(1664, 362, 903, 1640), newname = "LWintAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

LWint_2018_4RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to4RG, groupnames = GroupNames4, maindir = "../BAYES/Output/", mixvec = winter_2018_mixnames[5:8], 
                          catchvec = c(1664, 362, 903, 1640), newname = "LWintAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

save_objects(objects = objects(pattern = "LWint_2018_"), path = "../Estimates objects/")
```
...
SummerRet1_2018_33RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = 1:33, groupnames = GroupNames33, maindir = "../BAYES/Output/", mixvec = summer_2018_mixnames[c(2,4,1,3)], 
                          catchvec = c(38087, 14219, 2797, 3993), newname = "SummerRet1AllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

SummerRet1_2018_18RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to18RG, groupnames = GroupNames18, maindir = "../BAYES/Output/", mixvec = summer_2018_mixnames[c(2,4,1,3)], 
                          catchvec = c(38087, 14219, 2797, 3993), newname = "SummerRet1AllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

SummerRet1_2018_8RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to8RG, groupnames = GroupNames8, maindir = "../BAYES/Output/", mixvec = summer_2018_mixnames[c(2,4,1,3)], 
                          catchvec = c(38087, 14219, 2797, 3993), newname = "SummerRet1AllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

SummerRet1_2018_4RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to4RG, groupnames = GroupNames4, maindir = "../BAYES/Output/", mixvec = summer_2018_mixnames[c(2,4,1,3)], 
                          catchvec = c(38087, 14219, 2797, 3993), newname = "SummerRet1AllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

save_objects(objects = objects(pattern = "SummerRet1_2018_"), path = "../Estimates objects/")
```
...
SummerRet2_2018_33RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = 1:33, groupnames = GroupNames33, maindir = "../BAYES/Output/", mixvec = summer_2018_mixnames[c(6,8,5,7)], 
                          catchvec = c(16907, 8649, 454, 1736), newname = "SummerRet2AllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

SummerRet2_2018_18RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to18RG, groupnames = GroupNames18, maindir = "../BAYES/Output/", mixvec = summer_2018_mixnames[c(6,8,5,7)], 
                          catchvec = c(16907, 8649, 454, 1736), newname = "SummerRet2AllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

SummerRet2_2018_8RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to8RG, groupnames = GroupNames8, maindir = "../BAYES/Output/", mixvec = summer_2018_mixnames[c(6,8,5,7)], 
                          catchvec = c(16907, 8649, 454, 1736), newname = "SummerRet2AllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

SummerRet2_2018_4RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to4RG, groupnames = GroupNames4, maindir = "../BAYES/Output/", mixvec = summer_2018_mixnames[c(6,8,5,7)], 
                          catchvec = c(16907, 8649, 454, 1736), newname = "SummerRet2AllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

save_objects(objects = objects(pattern = "SummerRet2_2018_"), path = "../Estimates objects/")
```
...
spring_2018_mixnames
```

Well shit, I forgot that we ran some stat areas as May + June. Also, from looking at some of the stat areas that we ran separately for May and June, there is a LOT of temporal variation (WCVI decreases, Andrew increases). This means that we probably should not use the May + June strata to represent just May, same goes for June.
...
SpringNO_2018_33RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = 1:33, groupnames = GroupNames33, maindir = "../BAYES/Output/", mixvec = spring_2018_mixnames[c(8,1,8,5,9,5,10,2)], 
                          catchvec = c(498, 236, 67, 497, 1230, 52, 892, 190), newname = "SpringNOAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

SpringNO_2018_18RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to18RG, groupnames = GroupNames18, maindir = "../BAYES/Output/", mixvec = spring_2018_mixnames[c(8,1,8,5,9,5,10,2)], 
                          catchvec = c(498, 236, 67, 497, 1230, 52, 892, 190), newname = "SpringNOAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

SpringNO_2018_8RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to8RG, groupnames = GroupNames8, maindir = "../BAYES/Output/", mixvec = spring_2018_mixnames[c(8,1,8,5,9,5,10,2)], 
                          catchvec = c(498, 236, 67, 497, 1230, 52, 892, 190), newname = "SpringNOAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

SpringNO_2018_4RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to4RG, groupnames = GroupNames4, maindir = "../BAYES/Output/", mixvec = spring_2018_mixnames[c(8,1,8,5,9,5,10,2)], 
                          catchvec = c(498, 236, 67, 497, 1230, 52, 892, 190), newname = "SpringNOAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

save_objects(objects = objects(pattern = "SpringNO_2018_"), path = "../Estimates objects/")
```
...
SpringSO_2018_33RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = 1:33, groupnames = GroupNames33, maindir = "../BAYES/Output/", mixvec = spring_2018_mixnames[c(4,7)], 
                          catchvec = c(424, 281), newname = "SpringSOAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

SpringSO_2018_18RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to18RG, groupnames = GroupNames18, maindir = "../BAYES/Output/", mixvec = spring_2018_mixnames[c(4,7)], 
                          catchvec = c(424, 281), newname = "SpringSOAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

SpringSO_2018_8RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to8RG, groupnames = GroupNames8, maindir = "../BAYES/Output/", mixvec = spring_2018_mixnames[c(4,7)], 
                          catchvec = c(424, 281), newname = "SpringSOAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

SpringSO_2018_4RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to4RG, groupnames = GroupNames4, maindir = "../BAYES/Output/", mixvec = spring_2018_mixnames[c(4,7)], 
                          catchvec = c(424, 281), newname = "SpringSOAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

save_objects(objects = objects(pattern = "SpringSO_2018_"), path = "../Estimates objects/")
```
...
SpringSI_2018_33RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = 1:33, groupnames = GroupNames33, maindir = "../BAYES/Output/", mixvec = spring_2018_mixnames[c(3,6)], 
                          catchvec = c(289, 2306), newname = "SpringSIAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

SpringSI_2018_18RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to18RG, groupnames = GroupNames18, maindir = "../BAYES/Output/", mixvec = spring_2018_mixnames[c(3,6)], 
                          catchvec = c(289, 2306), newname = "SpringSIAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

SpringSI_2018_8RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to8RG, groupnames = GroupNames8, maindir = "../BAYES/Output/", mixvec = spring_2018_mixnames[c(3,6)], 
                          catchvec = c(289, 2306), newname = "SpringSIAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

SpringSI_2018_4RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to4RG, groupnames = GroupNames4, maindir = "../BAYES/Output/", mixvec = spring_2018_mixnames[c(3,6)], 
                          catchvec = c(289, 2306), newname = "SpringSIAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

save_objects(objects = objects(pattern = "SpringSI_2018_"), path = "../Estimates objects/")
```
...
Spring_2018_33RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = 1:33, groupnames = GroupNames33, maindir = "../BAYES/Output/", mixvec = spring_2018_mixnames[c(3,6,4,7,8,1,8,5,9,5,10,2)], 
                          catchvec = c(289, 2306, 424, 281, 498, 236, 67, 497, 1230, 52, 892, 190), newname = "SpringAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

Spring_2018_18RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to18RG, groupnames = GroupNames18, maindir = "../BAYES/Output/", mixvec = spring_2018_mixnames[c(3,6,4,7,8,1,8,5,9,5,10,2)], 
                          catchvec = c(289, 2306, 424, 281, 498, 236, 67, 497, 1230, 52, 892, 190), newname = "SpringAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

Spring_2018_8RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to8RG, groupnames = GroupNames8, maindir = "../BAYES/Output/", mixvec = spring_2018_mixnames[c(3,6,4,7,8,1,8,5,9,5,10,2)], 
                          catchvec = c(289, 2306, 424, 281, 498, 236, 67, 497, 1230, 52, 892, 190), newname = "SpringAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

Spring_2018_4RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to4RG, groupnames = GroupNames4, maindir = "../BAYES/Output/", mixvec = spring_2018_mixnames[c(3,6,4,7,8,1,8,5,9,5,10,2)], 
                          catchvec = c(289, 2306, 424, 281, 498, 236, 67, 497, 1230, 52, 892, 190), newname = "SpringAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

save_objects(objects = objects(pattern = "Spring_2018_"), path = "../Estimates objects/")
```

## All year, publication tables

```{r}
names(Troll_2018_Mixtures_8RG_EstimatesStats)
objects(pattern = "_8RG_Stratified")
```

```{r}
Troll2018_8RG_StratifiedEstimatesStats <- list("EWintAllQuad_2018" = EWint_2018_8RG_StratifiedEstimatesStats,
                                               "LWintAllQuad_2018" = LWint_2018_8RG_StratifiedEstimatesStats,
                                               "SpringAllQuad_2018" = Spring_2018_8RG_StratifiedEstimatesStats,
                                               "SumRet1AllQuad_2018" = SummerRet1_2018_8RG_StratifiedEstimatesStats,
                                               "SumRet2AllQuad_2018" = SummerRet2_2018_8RG_StratifiedEstimatesStats)
dput(x = Troll2018_8RG_StratifiedEstimatesStats, file = "../Estimates objects/Troll2018_8RG_StratifiedEstimatesStats.txt")

Troll2018_8RG_EstimatesStats <- list(
  "EWintNO_2018" = Troll_2018_Mixtures_8RG_EstimatesStats[["EWintNO_2018"]],
  "EWintAllQuad_2018" = EWint_2018_8RG_StratifiedEstimatesStats,
  "LWintNO_2018" = Troll_2018_Mixtures_8RG_EstimatesStats[["LWintNO_2018"]],
  "LWintAllQuad_2018" = LWint_2018_8RG_StratifiedEstimatesStats,
  "SpringNO_2018" = SpringNO_2018_8RG_StratifiedEstimatesStats,
  "SpringSI_2018" = SpringSI_2018_8RG_StratifiedEstimatesStats,
  "SpringAllQuad_2018" = Spring_2018_8RG_StratifiedEstimatesStats,
  "SumRet1NO_2018" = Troll_2018_Mixtures_8RG_EstimatesStats[["SummerRet1NO_2018"]],
  "SumRet1AllQuad_2018" = SummerRet1_2018_8RG_StratifiedEstimatesStats,
  "SumRet2NO_2018" = Troll_2018_Mixtures_8RG_EstimatesStats[["SummerRet2NO_2018"]],
  "SumRet2AllQuad_2018" = SummerRet2_2018_8RG_StratifiedEstimatesStats
  )
dput(x = Troll2018_8RG_EstimatesStats, file = "../Estimates objects/Troll2018_8RG_EstimatesStats.txt")
```
...
troll_2018_8RG_estimates.tdy <- 
  bind_rows(
    lapply(names(Troll2018_8RG_EstimatesStats), function(mix) {  # loop over mixture names
      Troll2018_8RG_EstimatesStats[[mix]] %>%  # for each matrix
        as_tibble(rownames = "group") %>%  # make tibble with rownames as group
        mutate(mixname = mix) %>%  # make column for mixname
        mutate(n_group = n_distinct(group))  # make column for number of groups
    } )
...
save_objects(c("troll_2018_8RG_estimates.tdy"), "../Estimates objects")
```
...
sum(c(4501, 86, 1262, 1549, 1664, 362, 903, 1640, 289, 2306, 424, 281, 498, 236, 67, 497, 1230, 52, 892, 190, 38087, 14219, 2797, 3993, 16907, 8649, 454, 1736))
```

Full AY stratified estimate
```{r troll_all_year_all_quad_stratified}
AllYearTroll2018_33RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = 1:33, groupnames = GroupNames33, maindir = "../BAYES/Output/", mixvec = c(winter_2018_mixnames, spring_2018_mixnames[c(3,6,4,7,8,1,8,5,9,5,10,2)], summer_2018_mixnames[c(2,4,1,3,6,8,5,7)]), 
...
AllYearTroll2018_18RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to18RG, groupnames = GroupNames18, maindir = "../BAYES/Output/", mixvec = c(winter_2018_mixnames, spring_2018_mixnames[c(3,6,4,7,8,1,8,5,9,5,10,2)], summer_2018_mixnames[c(2,4,1,3,6,8,5,7)]), 
                          catchvec = c(4501, 86, 1262, 1549, 1664, 362, 903, 1640, 289, 2306, 424, 281, 498, 236, 67, 497, 1230, 52, 892, 190, 38087, 14219, 2797, 3993, 16907, 8649, 454, 1736), newname = "AllYearAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)
...
AllYearTroll2018_4RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to4RG, groupnames = GroupNames4, maindir = "../BAYES/Output/", mixvec = c(winter_2018_mixnames, spring_2018_mixnames[c(3,6,4,7,8,1,8,5,9,5,10,2)], summer_2018_mixnames[c(2,4,1,3,6,8,5,7)]), 
                          catchvec = c(4501, 86, 1262, 1549, 1664, 362, 903, 1640, 289, 2306, 424, 281, 498, 236, 67, 497, 1230, 52, 892, 190, 38087, 14219, 2797, 3993, 16907, 8649, 454, 1736), newname = "AllYearAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

save_objects(objects = objects(pattern = "AllYearTroll2018_"), path = "../Estimates objects/")
```
...
```{r sport_mixtures}
Sport_2018_Mixtures_33RG_EstimatesStats <- 
  CustomCombineBAYESOutput.GCL(groupvec = 1:33, groupnames = GroupNames33, maindir = "../BAYES/Output", mixvec = sport_2018_mixnames,
                               prior = "", ext = "RGN", nchains = 5, burn = 0.5, alpha = 0.1, PosteriorOutput = FALSE)

Sport_2018_Mixtures_18RG_EstimatesStats <- 
  CustomCombineBAYESOutput.GCL(groupvec = GroupVec33RG_to18RG, groupnames = GroupNames18, maindir = "../BAYES/Output", mixvec = sport_2018_mixnames,
                               prior = "", ext = "RGN", nchains = 5, burn = 0.5, alpha = 0.1, PosteriorOutput = FALSE)

Sport_2018_Mixtures_8RG_EstimatesStats <- 
  CustomCombineBAYESOutput.GCL(groupvec = GroupVec33RG_to8RG, groupnames = GroupNames8, maindir = "../BAYES/Output", mixvec = sport_2018_mixnames,
                               prior = "", ext = "RGN", nchains = 5, burn = 0.5, alpha = 0.1, PosteriorOutput = FALSE)

Sport_2018_Mixtures_4RG_EstimatesStats <- 
  CustomCombineBAYESOutput.GCL(groupvec = GroupVec33RG_to4RG, groupnames = GroupNames4, maindir = "../BAYES/Output", mixvec = sport_2018_mixnames,
                               prior = "", ext = "RGN", nchains = 5, burn = 0.5, alpha = 0.1, PosteriorOutput = FALSE)

save_objects(objects = "sport_2018_mixnames", path = "../Objects/")
save_objects(objects = objects(pattern = "Sport_2018_Mixtures"), path = "../Estimates objects/")
```
...
sport_2018_8RG_estimates.tdy <- 
  bind_rows(
    lapply(sport_2018_mixnames, function(mix) {  # loop over mixture names
      Sport_2018_Mixtures_8RG_EstimatesStats[[mix]] %>%  # for each matrix
        as_tibble(rownames = "group") %>%  # make tibble with rownames as group
        mutate(mixname = mix) %>%  # make column for mixname
        mutate(n_group = n_distinct(group))  # make column for number of groups
    } )
...
save_objects(c("sport_2018_8RG_estimates.tdy"), "../Estimates objects")
```
...
AllYearSport2018_33RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = 1:33, groupnames = GroupNames33, maindir = "../BAYES/Output/", mixvec = sport_2018_mixnames[-c(1:2)], 
                          catchvec = c(5312, 889, 2618, 9328, 6475), newname = "AllYearAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

AllYearSport2018_18RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to18RG, groupnames = GroupNames18, maindir = "../BAYES/Output/", mixvec = sport_2018_mixnames[-c(1:2)], 
                          catchvec = c(5312, 889, 2618, 9328, 6475), newname = "AllYearAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

AllYearSport2018_8RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to8RG, groupnames = GroupNames8, maindir = "../BAYES/Output/", mixvec = sport_2018_mixnames[-c(1:2)], 
                          catchvec = c(5312, 889, 2618, 9328, 6475), newname = "AllYearAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

AllYearSport2018_4RG_StratifiedEstimatesStats <- 
  StratifiedEstimator.GCL(groupvec = GroupVec33RG_to4RG, groupnames = GroupNames4, maindir = "../BAYES/Output/", mixvec = sport_2018_mixnames[-c(1:2)], 
                          catchvec = c(5312, 889, 2618, 9328, 6475), newname = "AllYearAllQuad", ext = "RGN", nchains = 5, xlxs = FALSE, PosteriorOutput = FALSE)

save_objects(objects = objects(pattern = "AllYearSport2018_"), path = "../Estimates objects/")
```

# Compare All Year Troll AY 2009-2018

Get all year stock comp for AY 2009-2018
```{r troll_all_year_all_quad_2009-2018_8RG}
AllYearTroll2009_2018_8RG_StratifiedEstimatesStats <- 
...
save_objects("AllYearTroll2009_2018_8RG_StratifiedEstimatesStats", "../Estimates objects/")
```

Make in to tibble
```{r troll_all_year_all_quad_2009-2018_8RG_tibble}
# make in to a tidy tibble (tall)
troll_AY_2009_2018_8RG_estimates.tdy <- 
  bind_rows(
    sapply(as.character(2009:2018), function(yr) {
      AllYearTroll2009_2018_8RG_StratifiedEstimatesStats[[yr]] %>% 
        as_tibble(rownames = "group") %>%  # make tibble with rownames as group
...
save_objects(c("troll_AY_2009_2018_8RG_estimates.tdy"), "../Estimates objects")
```
...
# Compare All Year Sport AY 2009-2018

Get all year stock comp for AY 2009-2018
```{r sport_all_year_all_quad_2009-2018_8RG}
AllYearSport2009_2018_8RG_StratifiedEstimatesStats <- 
...
save_objects("AllYearSport2009_2018_8RG_StratifiedEstimatesStats", "../Estimates objects/")
```
...
sport_AY_2009_2018_8RG_estimates.tdy <- 
  bind_rows(
    sapply(as.character(2009:2018), function(yr) {
      AllYearSport2009_2018_8RG_StratifiedEstimatesStats[[yr]] %>% 
        as_tibble(rownames = "group") %>%  # make tibble with rownames as group
        mutate(year = as.integer(yr)) %>% # make column for year
        mutate(n_group = n_distinct(group))  # make column for number of groups
    }, simplify = FALSE)
...
save_objects(c("sport_AY_2009_2018_8RG_estimates.tdy"), "../Estimates objects")
```
...
```{r annual_summaries}
write_csv(x = troll_AY_2009_2018_8RG_estimates.tdy, path = "../Tables/troll_AY_2009_2018_8RG_annual_estimates.csv")
write_csv(x = sport_AY_2009_2018_8RG_estimates.tdy, path = "../Tables/sport_AY_2009_2018_8RG_annual_estimates.csv")
write_csv(x = annual_sport_harvest_AY_2009_2018, path = "../Tables/annual_sport_harvest_AY_2009_2018.csv")
write_csv(x = annual_troll_harvest_AY_2009_2018, path = "../Tables/annual_troll_harvest_AY_2009_2018.csv")
```
